...
export default function App() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      const savedSession = localStorage.getItem('fswd_user_session');
      if (savedSession) {
        try {
          ...
          const docRef = doc(db, 'progress', parsedUser.id);
          ...
          console.error("Failed to load session or progress from Firestore:", e);
          setUser(null);
          localStorage.removeItem('fswd_user_session');
        }
      } else {
  ...
  const toggleTaskCompletion = async (milestoneId: number, taskIdx: number) => {
    ...
    if (user) {
      ...
      try {
        ...
        await setDoc(docRef, { completedTasks: updated }, { merge: true });
      } catch (e) {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'login') {
        // Authenticate using custom Firestore collection
        ...
        const sessionUser = { id: uid, username: userData.username };
        setUser(sessionUser);
        localStorage.setItem('fswd_user_session', JSON.stringify(sessionUser));
        ...
        // Sync any local guest progress to database
        if (completedTasks.length > 0) {
          const progressDocRef = doc(db, 'progress', uid);
          await setDoc(progressDocRef, { completedTasks }, { merge: true });
        }

        const sessionUser = { id: uid, username: authUsername.trim() };
        setUser(sessionUser);
        localStorage.setItem('fswd_user_session', JSON.stringify(sessionUser));
      }
